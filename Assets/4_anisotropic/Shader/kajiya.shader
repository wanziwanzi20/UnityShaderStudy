    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.tDirWS = normalize(mul(unity_ObjectToWorld, float4(v.tangent.xyz, 0.0)).xyz); // 副切线方向 OS>WS
                //o.bDirWS = cross(o.nDirWS, o.tDirWS) * v.tangent.w;  // 切线方向 
                return o;                                       // 将输出结构 输出
            }
            // 输出结构>>>像素
            float4 frag(VertexOutput i) : COLOR {
                ...
                float uvBias = (ver_NoiseTex - 0.5) * _WarpInt * i.tDirWS.z;     // 计算UV偏移值/法线沿副切线方向偏移量
